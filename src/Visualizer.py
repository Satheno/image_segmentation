...
class CurveVisualizer(Visualizer):
    ...
    def _color_point(self, colors, filled, clustering_truth, cluster_idx, point, color, offset=np.array([0, 0, 0])):
        """
    ...
    def _color_points(self, instance, filled, colors):
        ...
        """
        clustering_truth = np.full(filled.shape, -100)
        for cluster_idx, (_, curve_points) in enumerate(instance.items()):
            ...
            for point in curve_points:
                colors, filled, clustering_truth = self._color_point(colors, filled, clustering_truth, cluster_idx,
                                                                     point, self._point_color)
                ...
                if self._noise:
                    for offset in [np.array([1, 0, 0]), np.array([0, 1, 0]), np.array([0, 0, 1]), np.array([-1, 0, 0]),
                        ...
                        colors, filled, clustering_truth = self._color_point(colors, filled, clustering_truth,
                                                                             cluster_idx, point, self._noise_color,
                                                                             offset=offset)
...
class VoronoiVisualizer(Visualizer):
    ...
    def _color_points_parallel(self, instance, filled, colors, base_voxel, voxel_offset, originals, originals_idxs,
        ...
        """
        cluster_truth = np.full(filled.shape, -100)
        dim = filled.shape[0]
            ...
            for y_idx in range(xy_dim):
                for x_idx in range(xy_dim):
                    ...
                    if min_dist < self._max_dist:
                        cluster_truth[z_idx][y_idx][x_idx] = -100  # because its a wall and therefore not in a cluster
